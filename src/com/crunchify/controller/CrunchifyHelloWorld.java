	  ...
	  public ModelAndView compare(@RequestParam("input1") String input1,  
	   ...
	   //    String output = "According to our Comparator, '" + input1 + "' is " + inEnglish + "'" + input2 + "'";  
	        ...
	        try {
				...
				PdfWriter.getInstance(document, new FileOutputStream("/home/ec2-user/tmp/cs549/ftp-test/createSamplePDF.pdf"));
				FileOutputStream fos = new FileOutputStream("/home/ec2-user/tmp/cs549/ftp-test/createSamplePDF.pdf");
				int index = 1; // 表格序号  
	            int page = 2;// 总共页数
				ByteArrayOutputStream baos[] = new ByteArrayOutputStream[page];
				String TemplatePDF ="/home/ec2-user/tmp/cs549/ftp-test/f1040nre.pdf";
			...
			//	 Map<String, Item> fieldMap = form.getFields();
				...
				//    } 
				    ...
				    form.setField("f1_05_0_", "123");  
				  
				    stamp.setFormFlattening(true); // 这句不能少 
				    stamp.close();
				    reader.close();
	   //     document.open(); 
	  //      document.add(new Paragraph(input1));
